...
class DefaultDirectoryScanner(
    ...
    private suspend fun startFolderProcessor(transaction: Transaction, folderChannel: Channel<File>) {
        folderChannel.receiveAsFlow().onEach { folderProcessor.process(transaction, it) }.flowOn(dispatchers.scanner)
            .collect {}
    }

    private suspend fun startImageProcessor(transaction: Transaction, imageChannel: Channel<File>) {
        imageChannel.receiveAsFlow().onEach { imageProcessor.process(transaction, it) }.flowOn(dispatchers.scanner)
            .collect {}
    }

    private suspend fun startVideoProcessor(transaction: Transaction, videoChannel: Channel<File>) {
        videoChannel.receiveAsFlow().onEach { videoProcessor.process(transaction, it) }.flowOn(dispatchers.scanner)
            .collect {}
    }
